class Matplotlib:
    ...
    def _justified(self, nrows, grid_arrangement):
        ax_specs = []
        num_small_cols = self.np.lcm.reduce(grid_arrangement)
        gs = self.gridspec.GridSpec(
            nrows, num_small_cols, figure=self.plt.figure(constrained_layout=True)
        )
    ...
    def _ragged(self, nrows, ncols, grid_arrangement):
        ...
        gs = self.gridspec.GridSpec(
            nrows, ncols * col_width, figure=self.plt.figure(constrained_layout=True)
        )
